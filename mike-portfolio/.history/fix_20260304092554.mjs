...
const responses = {
  rag: ['RAG pipeline:','-> Semantic chunking','-> Vector DB storage','-> Hybrid retrieval','-> MMR reranking','-> Prompt orchestration'].join('\n'),
  aws: ['AWS work:','-> Multi-AZ with ALB','-> Auto Scaling Groups','-> 99.9% uptime SLA','-> CodePipeline CI/CD'].join('\n'),
  agent: ['Agentic system:','-> Planner decomposes tasks','-> Executors call tools','-> Critic validates','-> Orchestrator manages state'].join('\n'),
  hire: ['Yes actively looking!','-> RAG production systems','-> AWS at scale','-> 26+ countries deployed','Email: mikencube03@gmail.com'].join('\n'),
  default: ['Mikes AI assistant!','Specializing in:','-> RAG architectures','-> Agentic AI','-> AWS infrastructure'].join('\n'),
};
...
function getResponse(q) {
  ...
  if (t.includes('agent') || t.includes('agentic')) return RESPONSES.agent;
  if (t.includes('hire') || t.includes('available')) return RESPONSES.hire;
  return RESPONSES.default;
}

function ChatWidget() {
  const [messages, setMessages] = useState([
    { role: 'ai', content: 'Hi! I am Mikes AI assistant. Ask me about RAG systems, agentic AI, or AWS infrastructure!' }
  ]);
  ...
  return (
    <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(0,102,255,0.3)', borderRadius: 16, overflow: 'hidden', boxShadow: '0 0 40px rgba(0,102,255,0.15)', width: '100%' }}>
      ...
      <div style={{ height: 240, overflowY: 'auto', padding: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
        {messages.map((msg, i) => (
...
export default function Home() {
  return (
    <main style={{ minHeight: '100vh', background: '#030303' }}>
      ...
      <motion.nav initial={{ y: -80, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.6 }} style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 32px', background: 'rgba(3,3,3,0.85)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
      ...
      <section id="about" style={{ minHeight: '100vh', display: 'grid', gridTemplateColumns: '1fr 1fr', alignItems: 'center', gap: 64, padding: '120px 64px 80px' }}>
        ...
        <motion.div initial={{ opacity: 0, x: -60 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8, delay: 0.2 }}>
          ...
          <h1 style={{ fontWeight: 900, lineHeight: 0.95, marginBottom: 16, fontSize: 'clamp(3rem,5vw,5rem)', letterSpacing: '-0.03em' }}>
          ...
          <p style={{ fontSize: 16, lineHeight: 1.8, marginBottom: 32, color: 'rgba(255,255,255,0.5)', maxWidth: 420 }}>I design and deploy scalable AI systems using RAG architectures, agentic workflows, and cloud infrastructure built for real enterprise environments.</p>
        ...
        <motion.div initial={{ opacity: 0, x: 60 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8, delay: 0.4 }} style={{ display: 'flex', flexDirection: 'column', gap: 20, alignItems: 'center' }}>
          <div style={{ position: 'relative', width: '100%', maxWidth: 300, height: 340, border: '2px solid rgba(0,102,255,0.4)', borderRadius: 16, overflow: 'hidden', boxShadow: '0 0 40px rgba(0,102,255,0.2)', flexShrink: 0 }}>
            <img src="/Mike_Org.jpeg" alt="Mike Ncube" style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'top center' }} />
            <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '12px 16px', background: 'linear-gradient(transparent, rgba(3,3,3,0.9))', display: 'flex', alignItems: 'center', gap: 8 }}>
              <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#00C896', boxShadow: '0 0 6px #00C896' }}></div>
              <span style={{ fontSize: 11, fontFamily: 'monospace', color: '#00C896', letterSpacing: '0.1em', textTransform: 'uppercase' }}>Available for Opportunities</span>
            </div>
          </div>
          <ChatWidget />
        </motion.div>
      ...
      <section id="projects" style={{ padding: '96px 64px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          {projects.map((p, i) => (
            <motion.div key={p.num} custom={i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={cardVariants} whileHover={{ y: -4 }} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid ' + p.color + '33', borderRadius: 16, padding: 24 }}>
      ...
      <section id="skills" style={{ padding: '96px 64px', background: 'rgba(255,255,255,0.01)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(240px,1fr))', gap: 16 }}>
          {skills.map((s, i) => (
            <motion.div key={s.category} custom={i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={cardVariants} whileHover={{ y: -4 }} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 16, padding: 24 }}>
...
console.log('Done! page.tsx written successfully.');
