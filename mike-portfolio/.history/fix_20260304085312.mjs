...
const skills = [
  { category: 'AI & LLM', icon: 'AI', items: ['RAG Architectures', 'Agentic AI', 'LangChain', 'Vector DBs', 'Prompt Engineering', 'Gemini CLI'] },
  { category: 'Cloud & Infra', icon: 'Cloud', items: ['AWS EC2/ALB', 'Auto Scaling', 'CI/CD', 'Terraform', 'Railway', 'VPC/IAM'] },
  { category: 'Data & ML', icon: 'Data', items: ['Apache Spark', 'Python', 'PostgreSQL', 'Pandas', 'Docker', 'ML Pipelines'] },
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#030303' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(12, 1fr)', gap: 16 }}>
          {projects.map((p, i) => (
            <motion.div key={p.num} custom={i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={cardVariants} whileHover={{ y: -4 }}
              style={{ gridColumn: 'span 12', background: 'rgba(255,255,255,0.03)', border: \`1px solid \${p.color}25\`, borderRadius: 16, padding: 24, cursor: 'pointer' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}>
                <span style={{ fontWeight: 900, fontSize: 56, color: \`\${p.color}20\`, lineHeight: 1 }}>{p.num}</span>
      ...
      <section id="skills" style={{ padding: '96px 64px', background: 'rgba(255,255,255,0.01)' }}>
        <h2 style={{ fontWeight: 900, color: '#fff', fontSize: 'clamp(2rem,4vw,3.5rem)', letterSpacing: '-0.02em', marginBottom: 48 }}>The Stack</h2>
      ...
      <section id="contact" style={{ padding: '96px 64px' }}>
        <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 24, padding: 64, textAlign: 'center', maxWidth: 640, margin: '0 auto' }}>
          <h2 style={{ fontWeight: 900, color: '#fff', fontSize: 'clamp(2rem,4vw,3rem)', letterSpacing: '-0.02em', marginBottom: 16 }}>Lets Build Something Real</h2>
        ...
        {loading && (
          <div style={{ display: 'flex', gap: 10 }}>
            ...
            <div style={{ padding: '8px 12px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '4px 12px 12px 12px', display: 'flex', gap: 4, alignItems: 'center' }}>
              <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#0066FF', animation: 'bounce 1.2s infinite' }}></div>
              <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#0066FF', animation: 'bounce 1.2s infinite', animationDelay: '0.2s' }}></div>
              <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#0066FF', animation: 'bounce 1.2s infinite', animationDelay: '0.4s' }}></div>
            </div>
          </div>
        )}
      </div>
...
console.log('Done! page.tsx written cleanly.');
```
